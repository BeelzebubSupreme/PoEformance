# PoEformance changelog / feature history

Chronological write-up of every shipped feature, fix, and reverse-engineering
finding — what changed, why, and the offsets / quirks discovered along the way.
This was split out of `CLAUDE.md` (which the assistant loads every turn) to keep
that file small; the content is **unchanged, just moved here**.

- **Search this file** when you need the background on an existing feature or a
  past RE result (offsets, gotchas, why something was done a certain way).
- **After shipping** a new feature/fix, append a dated `## <title> (shipped
  0.45.x.y)` section here (same style as the entries below), not in `CLAUDE.md`.
- Durable, cross-cutting conventions (structure, AHK v2 init gotcha, line
  endings, UI conventions, static verification) stay in `CLAUDE.md`.
